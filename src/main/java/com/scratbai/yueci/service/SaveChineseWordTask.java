...
private void saveWordData() {
	...
	try {
		ChineseWord cWord = mapper.readValue(protoData, ChineseWord.class);
		saveMp3ToCDN(cWord);
		userDao.addChineseWord(cWord);
...
private String generateMp3SaveKey(String wordName, int symbolIndex) {
	...
	return String.format("pronounce/%s/%d/%s.mp3", wordName,
			symbolIndex, mp3MD5);
}
